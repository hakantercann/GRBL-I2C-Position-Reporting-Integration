...
// I2C timing
// I2C Configuration - Adjust these values
#define I2C_REPORT_INTERVAL_MS 500    // Increased to 500ms (less frequent)
#define I2C_MAX_RETRIES 3             // Maximum retry attempts
#define I2C_ENABLE_PIN_CHECK 1        // Enable pin availability check
// State tracking for non-blocking operation
static uint32_t last_i2c_send_time = 0;
static uint8_t i2c_retry_count = 0;
static bool i2c_enabled = false;

static char line[LINE_BUFFER_SIZE]; // Line to be executed. Zero-terminated.

// Initialize I2C safely
static void init_i2c_safe() {
  #if I2C_ENABLE_PIN_CHECK
    // Check if pins are available (not used by other GRBL functions)
    // This is a basic check - you might need more sophisticated detection
    if (bit_istrue(settings.flags, BITFLAG_INVERT_PROBE_PIN)) {
      // A5 is used for probe, don't initialize I2C
      i2c_enabled = false;
      return;
    }
  #endif
...
// Non-blocking I2C send with error handling
static bool send_position_safe() {
  if (!i2c_enabled) return false;
  
  // Quick check - if system is in critical state, skip I2C
  if (sys.state == STATE_ALARM || sys.abort) {
    return false;
  }
  
  int32_t pos[N_AXIS];
  memcpy(pos, sys.position, sizeof(pos));
  
  // This should be made non-blocking in a real implementation
  // For now, we'll limit retries to minimize blocking time
  for (uint8_t retry = 0; retry < I2C_MAX_RETRIES; retry++) {
    // In a real implementation, you'd want to make send_steps_via_i2c non-blocking
    send_steps_via_i2c(pos, N_AXIS);
    // Assume success for now - in real implementation, check ACK
    return true;
  }
  
  return false; // Failed after retries
}
// Safe I2C update function with minimal overhead
void update_i2c_data_safe() {
  // Skip if I2C is not enabled
  if (!i2c_enabled) return;
  
  // Use millisecond counter if available, otherwise use a simple counter
  static uint16_t call_counter = 0;
  call_counter++;
  
  // Send I2C data at specified intervals
  if (call_counter >= I2C_REPORT_INTERVAL_MS) {
    call_counter = 0;
    
    // Only send if system is in a safe state
...
void protocol_main_loop()
{
  ...
  for (;;) {
    ...
    // Safe I2C data transmission - moved to less critical location
    // Only call this when not processing time-critical operations
...
// Optional: Add I2C status to GRBL status reports
void report_i2c_status() {
  if (i2c_enabled) {
    printPgmString(PSTR("I2C:enabled"));
  } else {
    printPgmString(PSTR("I2C:disabled"));
  }
}
